...
- **동작 방식**
  - 1. IOT 장비가 방전 시 서버에 충전 요청 신호 전송
  - 2. 관리 서버가 충전 가능한 드론을 선택해 출발 명령
  - 3. 드론이 지정 좌표(x, y)로 이동
  - 4. 현장 도착 후 무선 충전 시작 (결합 방식 고려 필요)
  - 5. 충전 완료 시 드론은 도킹 스테이션으로 복귀
   
- **기대 효과**
  - 산악 및 오지 등 접근이 어려운 IOT 장비 전력 관리 가능
  - 장거리 센서망 유지, 환경 모니터링 등 다양한 응용 가능능
